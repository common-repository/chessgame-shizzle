					...
					if ( $_FILES['start_import_cs_file']['size'] > ( 4096000 ) ) { // Can't be larger than 4 MB
						...
						if ( file_exists( $_FILES['start_import_cs_file']['tmp_name'] . '.pgn' ) ) {
							...
							foreach ( $gamelist as $key => $game ) {
								...
								if (isset($game['moves'])) {
									...
									foreach ( $game['moves'] as $move ) {
										...
										if ( isset( $move['comment'] ) ) {
											// content of post
											if ( $move_counter < 5 && $cs_content === '' ) {
												$cs_content = chessgame_shizzle_sanitize_content( $move['c    omment'] );
											}
										}
										// next...
										$move_counter++;
								...
								if ( $cs_content === '' ) {
									$cs_content = apply_filters( 'chessgame_shizzle_no_content_text', esc_html__('(No     content to display)', 'chessgame-shizzle') );
								}
